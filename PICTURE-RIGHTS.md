...
## The flags travel with the app

*CEO, 21 Sep 2026: "when off line the flags don't appear, the flags should be
pre-loaded in the app."*

Every flag the bank shows has a copy under `img/flags/`, taken from the same
Wikimedia Commons file the question cites, at the width the app displays. The
question and the picture registry keep the Commons address, licence, credit and
file page unchanged — that is what the check above verifies, and the credit
line still links to the Commons page. The app only swaps the address for the
local copy at the moment it draws the flag (`src/flags.js`, generated by
`curio-hq/tools/bundle_flags.js`), and the offline worker pre-loads every copy
on install. The rights are the rights of the Commons file: public domain for
every national flag shipped so far, which the check confirms against Commons on
every run.
