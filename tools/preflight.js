 ...
 * don't appear, the flags should be pre-loaded in the app." Every Commons flag
 * the bank or the picture registry cites must have a bundled copy on disk,
 * mapped in src/flags.js and named in the worker's FLAGS list — or a reader
 * offline gets "Which country's flag is this?" with no flag. The list is
 * generated by curio-hq/tools/bundle_flags.js; this refuses a build where the
 * three have drifted apart. */
try {
  const vm = require('vm');
  const sb = { window: {} }; vm.createContext(sb);
  for (const f of ['flags.js', 'entities.img.js', 'questions.js']) vm.runInContext(read(path.join(SRC, f)), sb, { filename: f });
  const FL = sb.window.CURIO_FLAGS || {};
  const fileOf = u => { const m = /\/commons\/(?:thumb\/)?[0-9a-f]\/[0-9a-f]{2}\/([^/?#]+)/.exec(String(u || '')); return m ? 'File:' + decodeURIComponent(m[1]) : null; };
  /* a flag: a Commons DRAWING (.svg) whose title says flag — the rule the bundler and the suite apply too */
  const isFlag = f => /flag/i.test(f) && /\.svg$/i.test(f);
  const cited = new Set();
  Object.values(sb.window.CURIO_IMAGES || {}).forEach(v => { const f = fileOf(v.u); if (f && isFlag(f)) cited.add(f); });
  (sb.window.CURIO_QUESTIONS || []).forEach(q => { const f = q.img && fileOf(q.img.u); if (f && isFlag(f)) cited.add(f); });
  const mapped = Object.values(FL);
  const unmapped = [...cited].filter(f => !FL[f]);
  const absent = mapped.filter(p => !fs.existsSync(path.join(ROOT, p)));
  const swFlags = new Set([...sw.matchAll(/"\.\/(img\/flags\/[^"]+)"/g)].map(m => m[1]));
  const notPreloaded = mapped.filter(p => !swFlags.has(p));
  const extra = [...swFlags].filter(p => !mapped.includes(p));
  if (unmapped.length) fail('flags cited but not bundled (a reader offline would see no flag)', unmapped.slice(0, 6).join(', ') + (unmapped.length > 6 ? ' +' + (unmapped.length - 6) + ' more' : '') + ' — run curio-hq/tools/bundle_flags.js');
